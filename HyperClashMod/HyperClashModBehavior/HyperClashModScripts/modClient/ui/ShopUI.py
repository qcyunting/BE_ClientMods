...
class ShopUI(ScreenNode):
    def __init__(self, namespace, name, param):
        ...
        # 路径映射
        # 指向 _common.scrolling_panel 模板内部的真实内容路径
    ...
    def _updateGoodsItem(self, gridchild, grid_row_data):
        ...
        # 补齐到3个
        while len(grid_row_data) < 3:
            grid_row_data.append(None)
        for i, good in enumerate(grid_row_data):
            ...
            if good is None or not good.get("name"):
                # 空格子：隐藏内容
                nameCtrl = getbaseui(base + "/label")
                if nameCtrl: nameCtrl.asLabel().SetText("")
                priceCtrl = getbaseui(base + "/lable_coin")
                if priceCtrl: priceCtrl.asLabel().SetText("")
                iconCtrl = getbaseui(base + "/icon/item")
                if iconCtrl: iconCtrl.asImage().SetSprite("textures/ui/transparent")
                btnCtrl = getbaseui("/button").asButton()
                btnCtrl.SetButtonTouchDownCallback(None)
                continue
    ...
    def UpdateShopData(self, data):
        self.shopData = data
        self.currentTab = data.get("current_tab", "")
        self.selectedItemId = data.get("selected_item_id", "")
