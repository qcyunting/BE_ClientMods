...
class NpcDialog(BaseSystem):
    ...
    def OpenDialogue(self, args):
        ...
        if dialogue_id is not None and npc_name is not None and npc_icon is not None and text is not None and buttons is not None and step_index>=0:
            if self.ui_npcdialog:
                clientApi.PopScreen()
            self.ui_npcdialog = clientApi.PushScreen(modName, 'npcdialog', {"isHud": 1, 'data': args, 'client': self})
        else:
